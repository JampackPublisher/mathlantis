...
const App: React.FC = () => {
  ...
  const fetchEncouragement = useCallback(async (solvedCount: number) => {
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `I have solved ${solvedCount} math problems in Mathlantis game. Give me a short, 1-sentence encouraging message as a Math Wizard. I am currently in the ${world} zone. CRITICAL: Do not use any emojis.`,
      });
      if (response.text) {
        setEncouragement(response.text.trim());
      }
    } catch (e) {
